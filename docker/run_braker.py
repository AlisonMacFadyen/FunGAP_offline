...
def run_braker(
    ...
    for bam_file in bam_files:
        ...
        if not os.path.exists(gff3_braker):
            augustus_bin_path = os.environ.get(
                'AUGUSTUS_BIN_PATH',
                os.path.dirname(D_CONF['AUGUSTUS_PATH'])
            )

            augustus_config_path = os.environ.get(
                'AUGUSTUS_CONFIG_PATH',
                os.path.join(os.path.dirname(D_CONF['AUGUSTUS_PATH']), '../config')
            )

            augustus_scripts_path = os.environ.get(
                'AUGUSTUS_SCRIPTS_PATH',
                os.path.join(os.path.dirname(D_CONF['AUGUSTUS_PATH']), '../scripts')
            )
