...
def main():
    ...
    parser.add_argument(
        ...
        help='BUSCO lineage dataset (must be available locally in offline mode)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--download_path', nargs='?', default=None,
        help='Path to local BUSCO datasets (required with --offline)'
    )
    parser.add_argument(
        '--offline', action='store_true',
        help='Run BUSCO in offline mode (do not attempt downloads)'
    )
    ...
    download_path = os.path.abspath(args.download_path) if args.download_path else None
    ...
    run_busco(input_fasta, lineage_dataset, output_dir, log_tup,
              offline=offline, download_path=download_path)
...
def run_busco(input_fasta, lineage_dataset, output_dir, log_tup,
              offline=False, download_path=None):
    '''Run BUSCO'''
